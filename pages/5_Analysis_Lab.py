...
<style>
.hero {
    background: linear-gradient(135deg, #f9fafb 0%, #e0e7ff 50%, #fef3c7 100%);
    padding: 40px 50px;
    border-radius: 28px;
    margin-bottom: 28px;
    text-align: center;
...
.hero h1 {font-size: 3rem; color: #4f46e5; margin: 0; text-shadow: 2px 2px 4px rgba(79,70,229,0.15);}
.hero p {color: #4338ca; font-size: 1.3rem; margin: 8px 0 0 0;}
</style>
""", unsafe_allow_html=True)

def cute_box(text: str, bg="#f8f4ff", emoji="✨"):
    st.markdown(f"""
    <div style='background:{bg};padding:18px 24px;border-radius:20px;border:2px solid #a5b4fc;line-height:1.7;box-shadow:0 3px 10px rgba(99,102,241,0.2);font-size:1.05rem;'>
        <span style='font-size:1.4rem;'>{emoji}</span> {text} <span style='font-size:1.4rem;'>✨</span>
    </div>
    """, unsafe_allow_html=True)

def info_card(title, content, color="#e0e7ff"):
    st.markdown(f"""
    <div style='background:{color};padding:20px;border-radius:16px;border-left:4px solid #6366f1;margin:12px 0;'>
        <h4 style='color:#4338ca;margin-top:0;'>{title}</h4>
...
if os.path.exists(logo_path):
    ...
    <div class='hero'>
      ...
      <p>Join <strong>Dr. Curio</strong> in becoming a data scientist and discovering hidden patterns in your data! 🔍✨</p>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
...
# Quick Tip
cute_box("💡 You're about to find connections in your data! Look for correlations and patterns - you're doing real science! 🧪", bg="#e0e7ff", emoji="🌟")

# Educational fun facts
st.markdown("### 🧪 Data Science Facts")
c1, c2, c3 = st.columns(3)
with c1:
    info_card("📈 What is Correlation?", "Correlation shows if two things change together! If one goes up, does the other go up too? That's correlation!", "#e0e7ff")
with c2:
...
opt = st.selectbox("🔍 Which dataset would you like to analyze?", 
                   ["Air & Weather 🌤️","Seeds & Growth 🌱","Pollinator Patrol 🐝"],
                   index=0)
df = air if "Air" in opt else (seeds if "Seeds" in opt else poll)

if df.empty:
    cute_box("🌈 No data yet! Go collect some observations first in the other modules, then come back here to analyze! 🌈", 
             bg="#e0e7ff", emoji="📊")
else:
    # Dataset stats
    ...
    if num.empty:
        cute_box("No numeric columns to analyze. Need numbers to find patterns!", bg="#fff7ed", emoji="💡")
    else:
        corr = num.corr()
        ...
        if strongest:
            ...
            if abs(val) > 0.7:
                msg = f"**🎯 Very Strong Relationship:** {i} and {j} are tightly connected! (r = {val:.2f}). When one changes, the other almost always changes the same way!"
            elif abs(val) > 0.5:
                msg = f"**✅ Moderate Relationship:** {i} and {j} tend to change together (r = {val:.2f}). There's definitely a pattern here!"
            else:
                msg = f"**📊 Weak Relationship:** {i} and {j} don't change much together (r = {val:.2f}). Keep exploring other pairs!"
            cute_box(msg, bg="#ecfccb", emoji="🔬")
    ...
    if len(cols) >= 2:
        ...
        if len(x) >= 2:
            ...
            **Direction:** When `{xcol}` goes {'UP' if m > 0 else 'DOWN'}, `{ycol}` tends to go {direction}!
            
            **Strength:** r² = {r2:.2f} means the line explains {r2*100:.0f}% of what's happening. {significance}
            
            **Real Talk:** {'This is a strong pattern you discovered!' if r2 > 0.5 else 'Keep collecting more data to see clearer patterns!'}
            """, bg="#e0e7ff", emoji="🧪")
            ...
            if len(x) > 0:
                ...
                st.markdown(f"**💡 Fun Prediction:** When {xcol} = {x_mean:.1f}, we'd predict {ycol} ≈ {y_pred_mean:.2f}")
        else:
            cute_box("Need at least 2 data points to make predictions! Keep collecting data! 📊", bg="#fff7ed", emoji="💡")
    else:
        cute_box("Pick a dataset with at least two numeric columns to run regression analysis! Need numbers to find patterns! 🔢", bg="#fff7ed", emoji="💡")
...
with tab1:
    st.markdown("""
    <div style='padding:20px;background:#e0e7ff;border-radius:16px;'>
...
with tab2:
    ...
    <div style='padding:20px;background:#fce7f3;border-radius:16px;'>
        ...
        Correlation measures how two variables change together. Do they go up together? Down together? Or stay independent?</p>
        <p><strong>➕ Positive Correlation:</strong> Both variables increase together! More sun = more butterflies ☀️🦋</p>
        <p><strong>➖ Negative Correlation:</strong> When one goes up, the other goes down! More pollution = fewer bees 🌫️🐝</p>
        ...
        <p><strong>🧠 Don't Confuse with Cause:</strong> Just because two things are related doesn't mean one causes the other! Correlation ≠ causation!</p>
...
with tab3:
    ...
    <div style='padding:20px;background:#ecfccb;border-radius:16px;'>
        <h3>🔮 Regression & Making Predictions</h3>
        <p><strong>🔮 What is Regression?</strong><br>
        Regression helps us predict! If we know one variable, can we predict another? That's regression in action!</p>
        ...
        <p><strong>⚠️ Limits:</strong> Predictions work best for data similar to what we trained on. Don't predict too far outside your data!</p>
