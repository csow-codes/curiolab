...
st.markdown("## 🐝 Pollinator Patrol — Bees & Butterflies")
cute_box("Observe for **10 minutes**. Count friendly pollinators. Be gentle — no touching or disturbing 🐝🦋")
...
with left:
    ...
    bees = st.slider("Bees seen (10 min)", 0, 30, 3)
    butterflies = st.slider("Butterflies seen (10 min)", 0, 30, 1)
...
with right:
    ...
    if df.empty:
        ...
        # simple categorical trend: average counts by weather/flowers
        st.markdown("#### Averages by condition")
        by_weather = df.groupby("weather")[["bees","butterflies"]].mean().round(1)
        st.dataframe(by_weather)
        by_flowers = df.groupby("flowers")[["bees","butterflies"]].mean().round(1)
        st.dataframe(by_flowers)
        ...
        def report(rows: pd.DataFrame) -> str:
            ...
            try:
                ...
                return f"AI unavailable: {e}. Compare sunny vs windy days manually."
...
st.write("Pollinators help plants make seeds and fruits. More flowers usually means more visitors. Wind and rain can lower activity. Try to observe at the same time each day.")
